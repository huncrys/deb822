...
package:
  ...
  WORKDIR /workspace/golang-github-dpeckett-deb822
  COPY . .
  ...
  RUN export DEBEMAIL="damian@pecke.tt" \
    ...
    && dch --create --package golang-github-dpeckett-deb822 --newversion "${VERSION}-1" \
      --distribution "UNRELEASED" --force-distribution  --controlmaint "Last Commit: $(git log -1 --pretty=format:'(%ai) %H %cn <%ce>')" \
    && tar -czf ../golang-github-dpeckett-deb822_${VERSION}.orig.tar.gz .
